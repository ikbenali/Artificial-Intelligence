...
class NbyMVacuumEnvironment(Environment) :
	...
	def execute_action(self, agent, action):
		...
		if action == 'Right':
			agent.location = (min(self.numRows - 1, agent.location[0] + 1), agent.location[1])
		...
		if action == 'Up':
			agent.location = (agent.location[0], min(self.numCols - 1, agent.location[1] + 1))

		if action == 'Down' :
			agent.location = (agent.location[0], max(0, agent.location[1] - 1))
